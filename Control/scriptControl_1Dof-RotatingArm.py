    ...
    * theta_function: o estado alvo [theta, omega] que o controlador deve
    ...
    def f_ode(self, t, y):
        ...
        if self.friction:
            ...
            derivs = [omega, 1 / I * (T - M * G * R * sin(theta))]
        return derivs
    ...
    def control_arm(self):
        ...
        torq = M * G * R * sin(theta_curr) + I * (
            alfa_target - 2 * lambda_ * omega_hat - (lambda_ ** 2) * theta_hat
        )
    ...
    def step(self, dt):
        ...
        if self.edo_method == 0:
            ...
            # Runge-Kutta 4 ordem
            kr45 = solve_ivp(self.f_ode, (t0, tf), self.state, t_eval=[tf])
...
# -------- EXECUÇÃO ---------#
# Inicializa a classe
manipulador = Manipulador1GdL(
    [0, 0], f_t, edo_method=1, friction=False, control=True, lamb=10
)
dt = 1.0 / 30  # Intevalor de tempo  = 30 fps
...
if animate_model:
    ...
    sim_ax = fig.add_subplot(
        pos_anim, aspect="equal", autoscale_on=False, xlim=(-1.5, 1.5), ylim=(-1.5, 1.5)
    )
    sim_ax.grid(alpha=0.8)
    sim_ax.set_title("Animação do Manipulador")

    torq_ax = fig.add_subplot(
        pos_torq,
        autoscale_on=False,
        xlim=(0, time_max),
        ylim=(manipulador.sat_limit[0], manipulador.sat_limit[1]),
    )
    torq_ax.set_title("Torque x Tempo")
    torq_ax.get_xaxis().set_visible(False)

    state_ax = fig.add_subplot(
        pos_state, autoscale_on=False, xlim=(0, time_max), ylim=(-8, 8)
    )
    state_ax.set_title(r"[$\theta,\dot{\theta}$] x Tempo")
    state_ax.get_xaxis().set_visible(False)

    error_ax = fig.add_subplot(
        pos_error, autoscale_on=False, xlim=(0, time_max), ylim=(-5, 5)
    )
    ...
    (line_error_omega,) = error_ax.plot(
        [], [], "y-", lw=2, label=r"$\dot{\hat{\theta}}$"
    )
...
if animate_model:
    ...
    interval = 1000 * dt - (
        t1 - t0
    )  # Intervalo baseado em dt e no tempo para se calcular um passo
    ...
    (line_error_theta,) = error_ax.plot(
        time_list, thetas_hat, "y-", lw=2, label=r"$\hat{\theta}$"
    )
    (line_omega,) = error2_ax.plot(
        time_list, omegas, "b-", lw=2, label=r"$\dot{\theta}$"
    )
    (line_error_omega,) = error2_ax.plot(
        time_list, omegas_hat, "c-", lw=2, label=r"$\dot{\hat{\theta}}$"
    )
    (line_erros_omega_theta,) = error3_ax.plot(
        omegas_hat, thetas_hat, "m-", lw=2, label=r"$\dot{\hat{\theta}}$"
    )
    error_ax.legend()
    error2_ax.legend()
